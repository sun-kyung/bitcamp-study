...
-- 강의실
DROP TABLE IF EXISTS t_lesson_room RESTRICT;

-- 강의실사진
DROP TABLE IF EXISTS t_lesson_room_photos RESTRICT;

-- 강의매니저
DROP TABLE IF EXISTS t_lesson_manager RESTRICT;
...
CREATE TABLE t_students (
  ...
  basic_adress    VARCHAR(100) NULL     COMMENT '기본주소', -- 기본주소
  detail_adress   VARCHAR(100) NULL     COMMENT '상세주소', -- 상세주소
  final_education VARCHAR(40)  NOT NULL COMMENT '최종학력', -- 최종학력
  maior           VARCHAR(40)  NOT NULL COMMENT '전공', -- 전공
...
CREATE TABLE t_lessons (
  ...
  id          INTEGER      NULL     COMMENT '강의실번호' -- 강의실번호
)
COMMENT '강의';
...
-- 강의실
ALTER TABLE t_lesson_room
  ADD CONSTRAINT PK_t_lesson_room -- 강의실 기본키
    PRIMARY KEY (
      lesson_room_no -- 강의실번호
    );

-- 강의실 유니크 인덱스
CREATE UNIQUE INDEX UIX_t_lesson_room
  ON t_lesson_room ( -- 강의실
    center ASC, -- 센터
    name ASC    -- 이름
  );
...
-- 강의매니저
ALTER TABLE t_lesson_manager
  ADD CONSTRAINT PK_t_lesson_manager -- 강의매니저 기본키
    PRIMARY KEY (
      lesson_no,  -- 강의번호
      manager_no  -- 회원번호
    );
...
-- 강의
ALTER TABLE t_lessons
  ADD CONSTRAINT FK_t_lesson_room_TO_t_lessons -- 강의실 -> 강의
    FOREIGN KEY (
      id -- 강의실번호
    )
    REFERENCES t_lesson_room ( -- 강의실
      lesson_room_no -- 강의실번호
    );
...
-- 강의실사진
ALTER TABLE t_lesson_room_photos
  ADD CONSTRAINT FK_t_lesson_room_TO_t_lesson_room_photos -- 강의실 -> 강의실사진
    FOREIGN KEY (
      lesson_room_no -- 강의실번호
    )
    REFERENCES t_lesson_room ( -- 강의실
...
-- 강의매니저
ALTER TABLE t_lesson_manager
  ADD CONSTRAINT FK_t_managers_TO_t_lesson_manager -- 매니저 -> 강의매니저
    FOREIGN KEY (
      manager_no -- 회원번호
    )
...
-- 강의매니저
ALTER TABLE t_lesson_manager
  ADD CONSTRAINT FK_t_lessons_TO_t_lesson_manager -- 강의 -> 강의매니저
    FOREIGN KEY (
      lesson_no -- 강의번호
    )
    REFERENCES t_lessons ( -- 강의
