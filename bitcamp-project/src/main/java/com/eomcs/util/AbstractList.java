...
// => 객체의 구현방법에 영향을 받지 않는 메서드를 따로 분리하여
//    객체의 사용규칙을 정의하게 되면
//    좀 더 유연하고 교체하기 쉬운 구조로 코드를 관리할 수 있다

// 초보자를 위한 실무 공식 :
// => 추상클래스를 정의한 다음 추상 메서드를 설정한다
// => 추상 메서드는 인터페이스 문법을 사용하여 별도로 분리한다
// => 클래스에서 사용 규칙(메서드 호출규칙)을 분리해서 따로 정의해두면
//    좀 더 다양한 하위클래스를 정의하고 다룰 수 있다
// => 추상 메서드가 대부분인 경우 인터페이스로 정의하는 것이 낫다
//    추상클래스의 목적이 하위 클래스에게 공통필드나 메서드를 상속해주는 것인데
//    대부분의 메서드가 추상메서드라서 서브클래스 입장에서 딱히 도움이 안 되는 상황일 때
//    그냥 메서드 사용 규칙만 정의하는 것이 코드관리에 낫기 때문이다

// 추상 메서드를 인터페이스로 분리했으면
// 다음과 같이 인터페이스 규칙을 추가하라
// => AbstractList를 상속받는 서브 클래스는
//    반드시 AbstractList에 추가된 규칙(List)에 따라 메서드를 정의해야 한다
//    즉 List에 선언된 모든 추상메서드를 반드시 구현(정의)해야 한다
// => 구현(implemnet)?
//    메서드의 몸체(body)를 정의하는 것
//    즉 메서드에 코드를 정의하는 것
//    메서드가 해야할 일이 무엇인지 코드를 정의하는 것
