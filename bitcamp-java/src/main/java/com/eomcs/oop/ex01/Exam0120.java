...
public class Exam0120 {
  public static void main(String[] args) {
    ...
    // => Score 설계도에 따라 만든 메모리의 주소를 저장할 변수를 만드는 명령
    // => 이렇게 메모리의 주소를 저장하는 변수를 "레퍼런스"라 부른다
    // => int와 같은 primitive data type인 경우 위와 같이 그냥 변수를 선언하면 되지만
    //    클래스로 설계한 메모리는 이런 방식으로 만들 수 없다
    ...
    // => 이렇게 설계도에 따라 준비된 메모리를 "인스턴스(instance)"라 한다.
