...
class Score2 {
  ...
  //sum이나 aver필드는 kor, eng, math값을 연산한 결과를 보관하기 때문에
  //직접 접근하여 값을 변경하는 것을 허용해서는 안 된다
  //허용하는 순간 개발자의 잘못된 명령으로 국영수 점수와 합계,평균이 서로 맞지 않는 문제가 발생할 수 있다
  //그래서 자바는 필드나 메서드의 외부접근범위를 조정하는 문법을 제공한다
  //그 문법을 '캡슐화(encapsulation)'이라 부른다
  
  private int sum;
  private float aver;
  
  //단 sum과 aver의 값을 직접 변경하지는 못하더라도 
  //외부에서 이 값들을 조회할 수 있는 방법/수단(method)은 제공해야 함
  // =>보통 이렇게 필드의 값을 조회하는 용도로 사용하는 메서드의 경우
  //   메서드의 용도를 이해하기 쉽도록  getXxx() 형태로 이름을 짓는다
  //   메서드의 이름이 get으로 시작한다고 해서 "게터(getter)"라고 짓는다
