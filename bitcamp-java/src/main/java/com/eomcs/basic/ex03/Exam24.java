...
public class Exam24 {
  public static void main(String[] args) {
    ...
    // -23
    // 1) Sign-magnitude(절대값) : 맨 앞 비트로 음수 표시
    ...
    //    단점 빼기 연산을 처리하는 컴퓨팅 회로를 별도로 설계해야 하므로 하드웨어 커짐
    //    장점 (인간이) 이해하기 쉽다
    ...
    //   +                                      1  <=1을 추가로 더한다
    //    ---------------------------------------
    //   10000 0000 0000 0000 0000 0000 0000 0000 = 0 (32비트가 넘어가는 건 버려짐)
    //   음수를 더할 때는 항상 결과에 1을 추가해야 한다

    // 3) 2's compliment(2의 보수)
    // => 1의 보수로 저장된 음수 값을 더할 때마다 계산 결과에 1을 추가하는 번거로움을
    //    없애기 위해 음수를 저장할 때 미리 1을 추가해 두는 방법
    // => 1의 보수 + 1 => 2의 보수
    ...
    //   그래서 컴퓨터에서 음수를 메모리에 저장할 때는 양수와 음수를 더할 때 정상적인
    //   값이 나오도록 2의 보수 방법으로 음수를 저장한다
    System.out.println(-23);
