...
public class Exam0110 {
  // 제네릭 문법이 없다면 각 타입의 객체를 저장하기 위해 다음과 같이 각 타입에 대한 클래스를 정의해야 한다
  ...
  public static void main(String[] args) {
    ...
    // Integer 객체를 저장하려면 IntegerBox를 사용해야 한다
    ...
    // 이렇게 객체를 저장하려면 각 객체의 타입별로 Box클래스를 생성해야 한다
    // 문제는 이런 식으로 코딩을 하면 타입 별로 만들어야 할 Box클래스가 무한정으로 늘어난다
    // 같은 일을 하는 클래스인데도 불구하고 다루는 객체의 타입이 다르다는 이유만으로
    // 여러 개의 유사 클래스를 반복적으로 정의해야하는 문제가 발생한다
    
    // 해결책
    // => 다양한 타입의 객체를 저장할 수 있도록 다형성의 다형적 변수 특징을 이용하여
    //    값을 저장하는 인스턴스 변수를 Object 타입으로 정의한다
    // => Exam0111.java소스를 확인해보라
  }
}
